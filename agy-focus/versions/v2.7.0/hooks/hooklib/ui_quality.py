...
DEFINITE_PATTERNS = (
    ...
    ("glass-blur", re.compile(r"(?:backdrop-filter|filter)\s*:[^;]*blur\s*\(", re.IGNORECASE), "유리·블러 장식을 제거하세요."),
...
def is_read_only_request(payload: dict) -> bool:
    ...
    return bool(READ_ONLY_TRIGGER_RE.search(prompt)) and not bool(WRITE_INTENT_RE.search(prompt))
