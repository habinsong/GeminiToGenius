...
UI_TRIGGER_RE = re.compile(
    ...
    r"component|responsive|microcopy|headline|copy)\b|디자인|화면|페이지|버튼|"
    r"레이아웃|반응형|랜딩|대시보드|컴포넌트|카피|문구|헤드라인)",
    re.IGNORECASE,
)
...
def save_state(
    ...
    *,
    ...
    try:
        ...
        if (
            conversation_id
            and previous.get("uiEvidenceConversationId") == conversation_id
        ):
            state["uiEvidenceConversationId"] = conversation_id
...
def ui_evidence_required(payload: dict) -> bool:
    return bool(UI_TRIGGER_RE.search(latest_user_prompt(payload)))


def ui_evidence_anchor_needed(payload: dict) -> bool:
    conversation_id = str(payload.get("conversationId", ""))
    if not conversation_id or not ui_evidence_required(payload):
        return False
    return read_state().get("uiEvidenceConversationId") != conversation_id


def visual_evidence_response(payload: dict) -> tuple[dict, dict | None]:
    if not ui_evidence_anchor_needed(payload):
        return {}, None
    return (
        {
            "injectSteps": [
                {
                    "ephemeralMessage": "UI·카피 작업: 모델이 만든 첫 화면·문구를 정답으로 승인하지 마세요. 실제 대상·사용자 행동·기존 디자인 시스템을 확인하고, 가능하면 실제 화면을 확인하세요. 화면 확인을 못 했으면 검증하지 못한 상태로만 보고하세요. 보라·파랑 그라데이션, 가짜 지표, 범용 AI 대시보드, 장식용 애니메이션, 추상 아이콘으로 빈 공간을 채우지 마세요."
                }
            ]
        },
        {"uiEvidenceConversationId": str(payload.get("conversationId", ""))},
    )
