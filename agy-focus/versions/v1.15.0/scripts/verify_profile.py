...
def main() -> int:
    ...
    if len(hooks) != 9:
        failures.append(f"expected 9 hooks, found {len(hooks)}")
    if len(skills) != 6:
        failures.append(f"expected 6 custom skills, found {len(skills)}")
    if workflows:
    ...
    if set(hooks) != expected_hooks:
        failures.append("hook names do not match the 9-hook contract")
    try:
    ...
    if set(skill_names) != expected_skills:
        failures.append(f"skill names do not match the 6-skill contract: {skill_names}")
