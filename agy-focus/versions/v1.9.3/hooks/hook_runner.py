...
UNSUPPORTED_OFFICIAL_CLAIM_RE = re.compile(
    ...
    r"\bpython\s+sdk\s+hooks\b|google\.antigravity\.hooks\.policy|"
    r"\bprogrammatic\s+logic\b|after\s+model\s+responses?\b|"
    ...
    if event == "stop-snapshot":
        ...
        if (
            ...
            emit(
                {
                    "decision": "continue",
                    "reason": "최신·공식 요청은 실제 search_web 호출, 공식 페이지 read_url_content 확인, URL 출처 보고가 있어야 완료됩니다. 읽은 본문에 직접 없는 분류·SDK API·기능명을 덧붙이지 말고 한 번 더 공식 페이지 기준으로 다시 작성하세요.",
                }
            )
            return
        save_state(event, payload, failed=bool(payload.get("error")))
