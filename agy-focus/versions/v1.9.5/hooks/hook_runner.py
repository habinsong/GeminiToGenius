    ...
    if event == "stop-snapshot":
        ...
        if (
            ...
            emit(
                {
                    "decision": "continue",
                    "reason": "최신·공식 요청은 실제 search_web 호출, 공식 페이지 read_url_content 확인, URL 출처 보고가 있어야 완료됩니다. canonical Hooks 페이지에 직접 없는 Inspect/Decide/Transform 분류, Python SDK, ask_user, before_tool_call, after_model_call을 보고하지 마세요. 페이지에 실제로 적힌 hooks.json, PreToolUse, PostToolUse, PreInvocation, PostInvocation, Stop과 필드만 기준으로 다시 작성하세요.",
                }
            )
            return
        save_state(event, payload, failed=bool(payload.get("error")))
