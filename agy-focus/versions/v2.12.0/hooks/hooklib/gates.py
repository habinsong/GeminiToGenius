...
from .ui_quality import is_read_only_request, is_strict_gtg, is_ui_task, ui_write_decision
...
def scope_read_gate(payload: dict) -> dict:
    ...
    target = write_target_path(payload)
    if is_strict_gtg(payload) and is_read_only_request(payload):
        if name in WRITE_TOOLS or (name == "run_command" and COMMAND_WRITE_RE.search(command_line(payload))):
            return {
                "decision": "deny",
                "reason": "읽기·검토·진단 요청에서는 명시적 변경 지시 없이 파일을 쓸 수 없습니다. 실제 소스·테스트·검증 결과만 확인하고 계획서·문서·코드 변경 제안을 적용하지 마세요.",
            }
