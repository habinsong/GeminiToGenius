        ...
        if isinstance(calls, list):
            for call in calls:
                if isinstance(call, dict) and isinstance(call.get("name"), str):
                    tool_names.append(call["name"].lower())
    required_tools = {"search_web", "read_url_content"}
    has_required_tools = required_tools.issubset(set(tool_names))
    has_url = bool(URL_RE.search("\n".join(text_parts)))
    return has_required_tools and has_url
