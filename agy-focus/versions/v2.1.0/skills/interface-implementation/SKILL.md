...
13. 웹 UI는 기본 제공 `scripts/verify_ui_render.py`로 320px·1280px 렌더와 수평 overflow를 계측하고 두 스크린샷을 직접 엽니다. 중간 폭과 키보드·터치·상태 전환도 조작합니다. 측정 가능한 환경에서 INP 200ms 이하와 CLS 0.1 이하를 확인하되 측정하지 않았으면 수치를 주장하지 않습니다.
14. 페이지 목적과 사용자 흐름이 둘 이상이면 다중 페이지를 계획하고 구현합니다. 각 경로는 고유한 `title`·`h1`·`main`과 실제 링크를 가져야 하며 `verify_multi_page.py`로 모든 경로를 320px·1280px에서 렌더합니다. `prefers-reduced-motion`과 보이는 `:focus-visible`가 없으면 완료로 쓰지 않습니다.
