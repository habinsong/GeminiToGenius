...
def decision_for(event: str, payload: dict) -> dict:
    ...
    if event == "destructive-command-gate":
        pattern = r"\b(rm\s+(-[A-Za-z]*r[A-Za-z]*|--recursive)|git\s+(reset\s+--hard|clean\b)|git\s+push\s+[^\n]*--force|sudo\b|mkfs\b|diskutil\s+erase\w*|chmod\s+-R)\b"
    ...
    if event == "external-input-gate":
        pattern = r"\b(curl|wget|git\s+(clone|fetch|pull|push)|npm\s+(install|i)|pnpm\s+(add|install)|yarn\s+(add|install)|pip\s+install|brew\s+install|uv\s+add|cargo\s+add|npx\s+skills\s+add)\b"
        if re.search(pattern, command, re.IGNORECASE):
            return {
                "decision": "force_ask",
...
def main() -> None:
    ...
    if event == "focus-anchor":
        ...
        steps = [
            {
                "ephemeralMessage": "집중 앵커: 현재 사용자 지시와 활성 작업창 상태만 추적하세요. 관련 없는 정리·플러그인·MCP 연결은 시작하지 말고, 변경 전 확인과 변경 후 검증을 끝낸 뒤에만 완료라고 판단하세요."
            }
        ]
        if failed:
            steps.append(
