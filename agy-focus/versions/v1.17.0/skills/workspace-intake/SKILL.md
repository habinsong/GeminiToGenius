...
2. 바이너리·캐시·의존성·빌드 산출물·비밀 파일은 제외 이유를 남깁니다. 영향 범위는 저장소 지침 → 빌드·실행 진입점 → 실제 소스·스크립트 → 호출·데이터·상태 경계의 관련 소스 → 테스트·fixture → 설정·관련 자산 → 문서 → README·계획·변경 기록 순서로 읽습니다.
3. 각 텍스트 파일은 `view_file`로 처음부터 끝까지 읽습니다. `rg`, 파일명 목록, 일부 줄, 검색 결과, 요약은 전체 읽기의 대체물이 아닙니다.
