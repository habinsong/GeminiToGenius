...
description: MUST invoke search_web and read_url_content to verify current, obscure, or official Antigravity and Gemini facts through primary-source pages before answering. Use automatically for 최신, 공식, 최신 문서, 검색, 찾아, 확인, verify, latest, docs, or source requests without an @mention, slash action, or manual skill selection.
---
...
1. `search_web`로 현재 연도(2026)와 `site:` 공식 도메인을 포함해 검색합니다.
...
7. 현재 공식 문서의 글로벌 스킬 경로는 `~/.gemini/config/skills`, workspace 스킬 경로는 `.agents/skills`로 취급합니다. `.gemini/skills`는 현재 공식 근거가 없으면 보고하지 않습니다.
8. 검색·URL 읽기 외의 MCP·플러그인·설치·파일 변경은 요청 범위에 없으면 하지 않습니다.
