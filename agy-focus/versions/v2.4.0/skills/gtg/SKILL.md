...
8. UI·웹 구현이면 실제 소스·테스트를 읽은 뒤 `docs/plans/design-plan.md`, `implementation-plan.md`, `verification-plan.md`를 먼저 작성하고 `verify_plan.py`를 통과시킵니다. 사용자 흐름이 둘 이상인 website·webpage는 `Route:`를 둘 이상 계획하고 다중 페이지로 만듭니다. 단일 페이지 선택에는 이유를 적습니다.
...
10. 최신·공식 사실은 현재 날짜로 검색하고 1차 출처 본문을 읽습니다. MCP·플러그인은 연결 자체가 작업 목적일 때만 사용합니다.
