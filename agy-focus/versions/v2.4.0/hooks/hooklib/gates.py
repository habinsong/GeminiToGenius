...
from .read_order import documentation_read_denied, tool_documentation_read_denied
...
def scope_read_gate(payload: dict) -> dict:
    ...
    if name not in {"run_command", "view_file", "read_url_content", "search_web", *WRITE_TOOLS}:
        return {"decision": "allow"}
    if name == "view_file":
