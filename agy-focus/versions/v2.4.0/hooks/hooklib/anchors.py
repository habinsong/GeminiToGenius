...
def scope_intake_response(payload: dict) -> tuple[dict, dict | None]:
    return one_time_anchor_response(
        ...
        CHANGE_TRIGGER_RE,
        "변경 작업: README·계획서·검색 결과부터 읽고 판단하지 마세요. 전체 파일 지도를 만든 뒤 저장소 지침과 빌드·실행 진입점을 확인하고, 영향 범위의 실제 소스·호출·데이터·상태 경계와 테스트를 파일별로 처음부터 끝까지 읽으세요. 긴 파일은 view_file 구간을 첫 줄부터 마지막 줄까지 틈 없이 이어 읽습니다. 설정·자산·문서는 마지막에 대조하세요. 목록·rg·일부 줄·요약은 전체 파일을 읽은 증거가 아니며 문서 설명은 실제 코드 동작의 증거가 아닙니다. 관련 없는 기능·과거 버전·생성물을 전부 주입해 집중력을 흩뜨리지 마세요.",
    )
