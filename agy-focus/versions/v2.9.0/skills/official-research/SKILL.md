...
5. `/GTG` UI 작업은 한 링크로 결론 내리지 않습니다. 플랫폼·적응형 레이아웃, 접근성, 성능, 인간 중심/에이전트 연구 중 최소 3개 분야에서 공식·1차 URL 4개 이상과 호스트 3개 이상을 읽고, 각 URL을 별도 matrix 행에 두어 발견·결정 또는 구현 영향·검증 게이트를 같은 행에 남깁니다.
