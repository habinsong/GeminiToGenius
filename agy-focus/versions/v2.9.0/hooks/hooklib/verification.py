...
def successful_helper(payload: dict, pattern: re.Pattern[str]) -> bool:
    ...
    for record in latest_request_records(payload):
        ...
        command = pending
        pending = None
        if pattern.search(command) and record.get("status") == "DONE" and re.search(r'["\']ok["\']\s*:\s*true', content, re.IGNORECASE):
            return True
    return False
...
def verification_after_last_write(payload: dict) -> bool:
    ...
    for record in latest_request_records(payload):
        ...
        if not pending:
            continue
        if record.get("type") == "RUN_COMMAND" and record.get("status") == "DONE":
            return True
        if (
            record.get("type") == "SYSTEM_MESSAGE"
            and record.get("status") == "DONE"
...
def visual_verification_status(payload: dict) -> tuple[bool, str]:
    ...
    if opened_screenshots >= required_screenshots:
        return True, ""
