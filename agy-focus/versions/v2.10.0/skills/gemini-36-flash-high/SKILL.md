...
16. 구현 뒤에는 렌더 명령만으로 끝내지 않습니다. 각 Route·상태를 Computer Use 또는 vision으로 실제 화면에서 관찰하고, 관찰 뒤 `search_web`와 공식 UI URL `read_url_content`를 실행해 외부 기준과 비교한 결과를 verification plan에 기록합니다. 이미지 생성·HTTP 200·소스 검토는 시각 관찰의 대체물이 아닙니다.
17. 계획의 flow trace·recovery path·변경 파일·Route를 구현과 대조하고, 결론·근거·검증·미확인만 보고합니다.
