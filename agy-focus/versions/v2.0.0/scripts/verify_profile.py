...
def main() -> int:
    ...
    try:
        ...
        ui_render_text = (CURRENT / "scripts/verify_ui_render.py").read_text(encoding="utf-8")
        ast.parse(ui_render_text)
        if len(ui_render_text.splitlines()) > MAX_HOOK_MODULE_LINES:
    ...
    if len(skills) != 10:
        failures.append(f"expected 10 custom skills, found {len(skills)}")
    if workflows:
    ...
    if set(skill_names) != expected_skills:
        failures.append(f"skill names do not match the 10-skill contract: {skill_names}")
