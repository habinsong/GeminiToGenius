#!/usr/bin/env python3
"""Render a page with installed Chrome and fail on horizontal clipping."""
...
VIEWPORTS = ((320, 800, "mobile"), (1280, 900, "desktop"))
CHROME_CANDIDATES = (
...
class DevTools:
    def __init__(self, websocket_url: str) -> None:
        ...
        self.stream = socket.create_connection((parsed.hostname or "127.0.0.1", parsed.port or 80), timeout=10)
...
def render_viewport(client: DevTools, url: str, output_dir: Path, width: int, height: int, label: str) -> dict:
    ...
    time.sleep(0.1)
    evaluated = client.call("Runtime.evaluate", {"expression": OVERFLOW_SCRIPT, "returnByValue": True})
    metrics = evaluated.get("result", {}).get("value")
    if not isinstance(metrics, dict):
        raise RuntimeError("Chrome did not return responsive layout metrics")
...
def verify(target: str, output_dir: Path, chrome: Path) -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="gtg-chrome-") as temporary:
        ...
        try:
            ...
            viewports = [
                render_viewport(client, url, output_dir, width, height, label)
            ...
            try:
                ...
                process.kill()
                process.wait(timeout=3)
    return {"ok": not any(item["overflow"] for item in viewports), "viewports": viewports}
