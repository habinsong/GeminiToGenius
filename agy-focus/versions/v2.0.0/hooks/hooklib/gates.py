...
def scope_read_status(payload: dict) -> dict:
    ...
    source_minimum = min(3, len(sources)) if code_task else 0
...
def scope_read_complete(payload: dict) -> bool:
    ...
    return (
        ...
        and (not status["testRequired"] or bool(status["testsViewed"]))
    )
...
def scope_read_reason(payload: dict) -> str:
    ...
    if len(status["sourcesViewed"]) < status["sourceMinimum"]:
        ...
        parts.append(
            f"실제 소스 전체 읽기 {len(source_viewed)}/{status['sourceMinimum']} 최소 근거. 후보: {preview}."
        )
    ...
    if status["testRequired"] and not status["testsViewed"]:
        ...
        parts.append(f"기존 테스트가 있습니다. 최소 한 파일을 끝까지 읽고 기대 동작을 확인하세요. 후보: {preview}.")
...
def documentation_read_gate(payload: dict) -> dict:
    ...
    tests_ready = not status["testRequired"] or bool(status["testsViewed"])
    if sources_ready and tests_ready:
        return {"decision": "allow"}
    return {
        "decision": "deny",
        "reason": "README·docs 선행 읽기를 차단했습니다. 저장소 지침과 manifest는 먼저 볼 수 있지만, "
        "현재 동작 판단은 실제 소스 최소 3개(더 적으면 전부)와 기존 관련 테스트 최소 1개를 "
...
def external_reference_gate(payload: dict) -> dict:
    ...
    return {
        "decision": "deny",
        "reason": "웹 검색·원격 README 선행 판단을 차단했습니다. 먼저 현재 workspace의 실제 소스 최소 3개"
        "(더 적으면 전부)와 기존 관련 테스트 최소 1개를 끝까지 읽고, 그 뒤 외부 문서와 대조하세요.",
    }
