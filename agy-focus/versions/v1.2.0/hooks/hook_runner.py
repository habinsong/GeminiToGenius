...
def main() -> None:
    ...
    if event == "focus-anchor":
        ...
        save_state(event, payload)
        try:
            profile_version = (CONFIG_ROOT / "current/VERSION").read_text(encoding="utf-8").strip()
        except OSError:
        ...
        steps = [
            {
                "ephemeralMessage": f"집중 앵커({profile_version}): @멘션·/액션 선택을 기다리지 말고 현재 자연어 지시를 답변·조회·변경·검색·UI·진단 중 하나로 자동 라우팅하세요. 현재 사용자 지시와 활성 작업창 상태만 추적하고, 관련 없는 정리·플러그인·MCP 연결은 시작하지 마세요. 변경 전 확인과 변경 후 검증을 끝낸 뒤에만 완료라고 판단하세요.{model_note}"
            }
        ]
        if failed:
            steps.append(
