...
def main() -> int:
    ...
    one_tap_workflow = CURRENT / "workflows/agy-one-tap.md"
    global_one_tap_workflow = GLOBAL_WORKFLOWS / "agy-one-tap.md"
    try:
        if not global_one_tap_workflow.is_symlink():
            failures.append("global workflow is not a symlink")
        if global_one_tap_workflow.resolve() != one_tap_workflow.resolve():
            failures.append("global workflow symlink mismatch")
        data = frontmatter(one_tap_workflow)
        if not data.get("description"):
            failures.append(f"workflow description missing: {one_tap_workflow}")
        if len(one_tap_workflow.read_text(encoding="utf-8")) >= 12000:
            failures.append("agy-one-tap workflow exceeds 12000 characters")
