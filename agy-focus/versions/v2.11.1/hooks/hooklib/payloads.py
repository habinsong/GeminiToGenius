...
from .coverage import fully_viewed_paths as covered_paths
from .coverage import path_coverage as covered_path_status
from .coverage import record_succeeded


TARGET_PATH_KEYS = (
    "TargetFile", "targetFile", "AbsolutePath", "absolutePath", "FilePath", "filePath", "path",
)
...
def command_line(payload: dict) -> str:
    ...
    for key in ("CommandLine", "commandLine", "command_line", "command", "cmd"):
        value = clean_text_argument(args.get(key))
        if value:
            return value
    return ""
...
def write_target_path(payload: dict) -> Path | None:
    call = payload.get("toolCall")
    return call_target_path(call) if isinstance(call, dict) else None


def call_target_path(call: dict) -> Path | None:
    args = call.get("args")
    if not isinstance(args, dict):
        return None
...
def workspace_roots(payload: dict) -> list[Path]:
    ...
    roots: list[Path] = []
    for value in values:
        value = clean_text_argument(value)
        if not value:
            continue
        try:
...
def transcript_tool_calls(payload: dict) -> list[dict]:
    ...
    for record in latest_request_records(payload):
        if not record_succeeded(record):
            continue
        calls.extend(record_tool_calls(record))
    return calls


def record_tool_calls(record: dict) -> list[dict]:
    calls: list[dict] = []
...
def indexed_tool_calls(payload: dict) -> list[tuple[int, dict]]:
    return list(enumerate(transcript_tool_calls(payload)))


def path_coverage(payload: dict, path: Path) -> tuple[int, int | None, bool]:
    return covered_path_status(transcript_tool_calls(payload), path)


def fully_viewed_paths(payload: dict) -> set[Path]:
    return covered_paths(transcript_tool_calls(payload))


def written_paths(payload: dict) -> set[Path]:
    result: set[Path] = set()
    write_tools = {"write_to_file", "replace_file_content", "multi_replace_file_content"}
    for call in transcript_tool_calls(payload):
        if str(call.get("name", "")).lower() not in write_tools:
            continue
...
def last_write_indexes(payload: dict) -> dict[Path, int]:
    result: dict[Path, int] = {}
    write_tools = {"write_to_file", "replace_file_content", "multi_replace_file_content"}
    for index, call in indexed_tool_calls(payload):
        if str(call.get("name", "")).lower() not in write_tools:
            continue
        path = call_target_path(call)
        if path:
