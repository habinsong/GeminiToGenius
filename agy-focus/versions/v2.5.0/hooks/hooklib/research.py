...
def research_evidence_present(payload: dict) -> bool:
    ...
    official_urls = [url for url in read_urls if OFFICIAL_UI_SOURCE_RE.search(url)]
    assistant_text = "\n".join(
        record.get("content", "")
        for record in records
    ...
    if ui_required:
        source_ok = len(official_urls) >= 2 and len(_distinct_hosts(official_urls)) >= 2
        report_ok = all(url in assistant_text for url in official_urls[:2])
    else:
        source_ok = any(expected_url.search(url) for url in read_urls)
        report_ok = bool(expected_url.search(assistant_text))
    return (
        {"SEARCH_WEB", "READ_URL_CONTENT"}.issubset(result_types)
        and source_ok
        and report_ok
