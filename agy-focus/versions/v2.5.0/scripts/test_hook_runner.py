...
def write_complete_plan(workspace: Path, *, routes: int = 2) -> None:
    ...
    route_lines = "\n".join(f"Route: /page-{index}" for index in range(1, routes + 1))
...
def test_strict_ui_research_requires_two_official_sources(runner) -> None:
    with tempfile.TemporaryDirectory() as temp:
        directory = Path(temp)
        records = [
            ...
            {"source": "MODEL", "content": "https://developer.apple.com/design/human-interface-guidelines/layout https://developer.chrome.com/docs/web-platform/long-animation-frames"},
        ]
...
def main() -> None:
    ...
    test_strict_ui_research_requires_two_official_sources(runner)
