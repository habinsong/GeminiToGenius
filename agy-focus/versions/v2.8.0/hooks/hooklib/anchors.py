...
def visual_evidence_response(payload: dict) -> tuple[dict, dict | None]:
    ...
    return one_time_anchor_response(
        ...
        quality_pattern,
        "UI·카피 결과: 첫 생성물을 승인하지 마세요. 마지막으로 쓴 파일을 다시 끝까지 읽고 직접 검증한 뒤 실제 화면을 좁은·중간·넓은 폭에서 조작하세요. 제품 고유 작업과 데이터가 위계에 반영되는지, loading·empty·error·success·disabled·focus·selected 상태가 있는지, 키보드·대비·타깃 크기·감속 설정·layout shift를 확인하세요. 화면을 보지 않았거나 성능을 재지 않았다면 통과와 수치를 주장하지 마세요.",
    )
