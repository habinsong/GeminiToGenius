...
def scope_intake_response(payload: dict) -> tuple[dict, dict | None]:
    return one_time_anchor_response(
        ...
        CHANGE_TRIGGER_RE,
        "변경 작업: README·계획서·검색 결과부터 읽고 판단하지 마세요. 전체 파일 지도를 만든 뒤 저장소 지침과 빌드·실행 진입점을 확인하고, 영향 범위의 실제 소스·호출·데이터·상태 경계와 테스트를 파일별로 처음부터 끝까지 읽으세요. 긴 파일은 view_file 구간을 첫 줄부터 마지막 줄까지 틈 없이 이어 읽습니다. 설정·자산·문서는 마지막에 대조하세요. `list_dir`를 포함한 목록·rg·일부 줄·요약은 전체 파일을 읽은 증거가 아니며 문서 설명은 실제 코드 동작의 증거가 아닙니다. 관련 없는 기능·과거 버전·생성물을 전부 주입해 집중력을 흩뜨리지 마세요.",
    )
...
def design_context_response(payload: dict) -> tuple[dict, dict | None]:
    return one_time_anchor_response(
        ...
        DESIGN_TRIGGER_RE,
        "UI 작업: 구현 전에 실제 사용자·주 행동·데이터·전체 상태·기존 화면과 디자인 시스템을 확인하세요. 제품 근거 없이 중앙 히어로+그라데이션 제목+목업+동일 카드 3개, 카드 중첩, 같은 radius·간격, pill·glow·blob·glass·blur, Inter/Roboto/Arial 기본값, 요술봉·별·추상 아이콘, 가짜 지표·후기·AI 배지·3D 장식, 목적 없는 hover/motion을 만들지 마세요. 이를 GitHub/Vercel풍 다크 문서·terminal·네오 브루탈리즘·긴 표 덤프로 치환하지 말고, 정적 페이지에 요청하지 않은 JavaScript를 넣지 마세요. 의미 구조·타이포그래피·intrinsic sizing·모바일 읽기 순서를 먼저 만들고 adaptive reflow/reveal/presentation 전략·compact 전환 조건·키보드 패턴을 계획에 남긴 뒤 320 CSS px·키보드 focus·색상 외 상태·감속 설정·좁은/중간/넓은 실제 렌더링까지 확인하세요.",
    )
...
def visual_evidence_response(payload: dict) -> tuple[dict, dict | None]:
    ...
    return one_time_anchor_response(
        ...
        quality_pattern,
        "UI·카피 결과: 첫 생성물을 승인하지 마세요. 마지막으로 쓴 파일을 다시 끝까지 읽고 직접 검증한 뒤 실제 화면을 좁은·중간·넓은 폭에서 조작하세요. 렌더 성공 뒤 Computer Use·vision으로 Route·상태를 관찰하고, 관찰 뒤 search_web와 공식 read_url_content로 외부 UI 기준을 다시 대조하세요. 제품 고유 작업과 데이터가 위계에 반영되는지, loading·empty·error·success·disabled·focus·selected 상태가 있는지, 키보드·대비·타깃 크기·감속 설정·layout shift를 확인하세요. 화면을 보지 않았거나 성능을 재지 않았다면 통과와 수치를 주장하지 마세요.",
    )
