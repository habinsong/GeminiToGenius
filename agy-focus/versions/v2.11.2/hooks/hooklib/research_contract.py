...
EVIDENCE_FIELDS = {
    "finding": re.compile(r"(?:key\s+)?finding|discovery|근거\s*발견|발견|관찰", re.IGNORECASE),
    "impact": re.compile(r"implementation\s+impact|decision|구현\s*영향|결정|적용", re.IGNORECASE),
    "verification": re.compile(r"verification(?:\s+gate)?|검증(?:\s+게이트)?|확인", re.IGNORECASE),
}
...
def source_categories(url: str) -> set[str]:
    ...
    if host in {
        "fluent2.microsoft.design", "atlassian.design", "s2.spectrum.adobe.com",
        "spectrum.adobe.com", "carbondesignsystem.com", "polaris-react.shopify.com",
        "polaris-site-prod-kit.shopify.prod.shopifyapps.com",
    }:
        categories.add("design-system")
...
def _evidence_record(text: str, url: str) -> str:
    """Return the local matrix row/block for one source URL."""
...
def evidence_matrix_status(text: str, urls: list[str]) -> dict:
    """Require each cited source to carry finding, impact, and verification locally."""

    records: dict[str, dict[str, bool]] = {}
    failures: list[str] = []
    for url in urls:
        record = _evidence_record(text, url)
        fields = {name: bool(pattern.search(record)) for name, pattern in EVIDENCE_FIELDS.items()}
        records[url] = fields
        missing = [name for name, present in fields.items() if not present]
        if missing:
            failures.append(f"matrix:{urlsplit(url).netloc}:{','.join(missing)}")
    return {"ok": not failures, "records": records, "failures": failures}
...
def plan_evidence_status(text: str) -> dict:
    ...
    matrix = evidence_matrix_status(text, status["officialUrls"])
    status["evidenceMatrix"] = matrix["records"]
    status["failures"].extend(matrix["failures"])
