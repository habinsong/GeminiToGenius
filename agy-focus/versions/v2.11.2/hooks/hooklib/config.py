...
READ_ONLY_TRIGGER_RE = re.compile(
    r"(?:(?:read[- ]?only|review[- ]?only|audit[- ]?only|diagnos(?:e|is)|inspect|review|explain|why|how|what|question|help|error|block(?:ed)?|stop|hook|reason|termination)|"
    r"읽기\s*전용|검토|진단|감사|분석|확인만|수정하지\s*(?:마|말고)|변경하지\s*(?:마|말고)|"
    r"절대\s+수정하지|설명해|이유|왜|어떻게|질문|알려줘|문의|알아봐|뭐야|무슨|오류|에러|막혀|블락|무한|루프|부족합니다|우회하지)",
    re.IGNORECASE,
)
