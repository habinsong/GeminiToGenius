...
GLOBAL_WORKFLOW_DIRS = (
    HOME / "antigravity/global_workflows",
    HOME / "antigravity-ide/global_workflows",
)
...
def main() -> int:
    ...
    try:
        ...
        for global_workflow_dir in GLOBAL_WORKFLOW_DIRS:
            global_workflow = global_workflow_dir / "agy-run.md"
            label = global_workflow_dir.parent.name
            if not global_workflow.is_symlink():
                failures.append(f"{label} global workflow is not a symlink")
            elif global_workflow.resolve() != workflow.resolve():
                failures.append(f"{label} global workflow symlink mismatch")
