...
- `python3 ~/.gemini/config/agy-focus/current/scripts/verify_ui_render.py <HTML> --output-dir <DIR>`로 320px·1280px 렌더, `scrollWidth`, 요소 경계, 문서·접근성 계약, CLS·Long Animation Frames를 검사하고 두 이미지를 직접 엽니다. `ok: true`가 아니면 잘린 요소를 고친 뒤 다시 실행합니다. HTTP 200·파서·`curl`은 시각 검증이 아닙니다. 측정하지 않은 INP·CLS·대비 수치를 주장하지 않습니다.
