...
def decision_for(event: str, payload: dict) -> dict:
    ...
    if event == "destructive-command-gate":
        pattern = r"\b(rm\s+(-[A-Za-z]*r|--recursive)|git\s+(reset|clean)|git\s+push\s+[^\n]*--force|sudo|mkfs|diskutil\s+eraseDisk|chmod\s+-R)\b"
    ...
    if event == "external-input-gate":
        pattern = r"\b(curl|wget|git\s+clone|npm\s+install|pnpm\s+add|yarn\s+add|pip\s+install|brew\s+install|npx\s+skills\s+add)\b"
        if re.search(pattern, command, re.IGNORECASE):
            return {
                "decision": "force_ask",
