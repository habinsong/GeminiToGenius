    ...
    if event == "stop-snapshot":
        ...
        if (
            ...
            emit(
                {
                    "decision": "continue",
                    "reason": "최신·공식 요청은 실제 search_web 호출, 공식 페이지 read_url_content 확인, URL 출처 보고가 있어야 완료됩니다. /GTG UI 작업은 공식 URL 4개 이상·서로 다른 호스트 3개 이상·플랫폼/접근성/성능/HCI 분야 3개 이상을 읽고 모든 URL을 보고해야 합니다. 계획의 결정·근거 발견·구현 영향·검증 게이트도 남기세요. canonical Hooks 페이지에 직접 없는 Inspect/Decide/Transform 분류, Python SDK, ask_user, before_tool_call, after_model_call, continue_on_error, event: 필드를 보고하지 마세요.",
                }
            )
            return
        save_state(event, payload, failed=bool(payload.get("error")))
