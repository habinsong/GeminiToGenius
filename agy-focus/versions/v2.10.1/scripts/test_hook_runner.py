...
def view_call(path: Path, **extra: object) -> dict:
    ...
    args.update(extra)
    return {"status": "DONE", "tool_calls": [{"name": "view_file", "args": args}]}
...
def write_complete_plan(workspace: Path, *, routes: int = 2) -> None:
    ...
    (plan_dir / "design-plan.md").write_text(
        ...
        + route_lines
        + "User outcome: the maintainer can choose the next action from the correct route and recover after a failed action.\n"
        + "Recovery: failed actions expose retry and undo without losing the current route or selection.\n"
        + "Decision ledger:\n| Decision ID | User outcome | Decision | Rationale / evidence | Implemented in | Verified by |\n"
        + "| --- | --- | --- | --- | --- | --- |\n"
        + "| D1 | Find the next action quickly | Use restrained hierarchy | Window-size guidance and task order | index.html, style.css | route screenshots and focus audit |\n"
        + "| D2 | Recover from a failed action | Preserve route and expose retry | GUIDE workflow context and explicit recovery | app.js, verification.html | primary flow trace and error-state check |\n"
        + """\nStates: loading, empty, error, success, disabled, selected, focus.\nResponsive: mobile first at 320 CSS px and wider layouts.\nAdaptive strategy: reflow, reveal, or presentation change at the full-screen to compact transition.\nKeyboard pattern: tabs use the WAI-ARIA tabs pattern and visible focus.\nVisual direction: restrained mode with editorial hierarchy and product-specific emphasis.\nRationale: typography and spacing expose the real content order.\nAlternative: compare an expressive navigation shell and record the tradeoff before choosing.\nRejected alternative: discard the expressive shell because it adds no task value.\nDecision: choose restrained hierarchy.\nEvidence finding: adaptive panes reduce navigation cost.\nImplementation impact: route layout and focus order change at the compact threshold.\nVerification gate: browser and keyboard checks at every route.\nMotion: explain a state change; trigger is route feedback, purpose is feedback, duration is 180ms, easing is ease-out, budget is 200ms.\nReduced motion: prefers-reduced-motion removes non-essential motion.\nAccessibility: keyboard order, visible focus, labels, target size.\nEvidence matrix:\n- Source: https://developer.apple.com/design/human-interface-guidelines/layout. Finding: layout adapts to window size. Implementation impact: route hierarchy changes at compact width. Verification gate: inspect 320 and 1280 screenshots.\n- Source: https://www.w3.org/TR/WCAG22/. Finding: reflow and focus remain operable. Implementation impact: preserve reading order and visible focus. Verification gate: keyboard and 320 CSS px checks.\n- Source: https://developer.chrome.com/docs/web-platform/long-animation-frames. Finding: long frames expose blocking work. Implementation impact: keep motion on compositor-friendly properties. Verification gate: record Long Animation Frames.\n- Source: https://research.google/pubs/guide-a-benchmark-for-user-context-understanding-and-assistance-in-gui-workflow-videos/. Finding: user context matters across workflow steps. Implementation impact: preserve route and state trace. Verification gate: replay the primary flow.\n""",
        encoding="utf-8",
    )
...
def test_evidence_matrix_requires_local_decisions(runner) -> None:
    ...
    sources = (
        ...
        "https://research.google/pubs/guide-a-benchmark-for-user-context-understanding-and-assistance-in-gui-workflow-videos/",
    )
    complete = "\n".join(
        f"Source: {url}. Finding: verified fact. Implementation impact: explicit decision. Verification gate: concrete check."
        for url in sources
    )
    assert plan_evidence_status(complete)["ok"]
    weak = complete.replace("Implementation impact: explicit decision.", "General note.")
    failed = plan_evidence_status(weak)
    assert not failed["ok"]
    assert any(failure.startswith("matrix:") for failure in failed["failures"])


def test_decision_ledger_requires_implementation_and_verification(runner) -> None:
    from hooklib.design_contract import decision_ledger_status

    complete = """Decision ledger:
| Decision ID | User outcome | Decision | Rationale / evidence | Implemented in | Verified by |
| --- | --- | --- | --- | --- | --- |
| D1 | Find the action | Use a clear primary action | User task evidence | index.html | flow trace |
| D2 | Recover safely | Keep retry visible | Error-state evidence | app.js | error test |
"""
    assert decision_ledger_status(complete)["ok"]
    weak = complete.replace(
        "| D2 | Recover safely | Keep retry visible | Error-state evidence | app.js | error test |",
        "| D2 |  | Keep retry visible | Error-state evidence | app.js | error test |",
    )
    failed = decision_ledger_status(weak)
    assert not failed["ok"]
    assert any("row-2-empty" in failure for failure in failed["failures"])


def test_contextual_material_gate(runner) -> None:
    from hooklib.ui_quality import quality_issues

    content_material = ".card { backdrop-filter: blur(18px); }"
    assert any(
        issue.startswith("content-blur:")
        for issue in quality_issues(content_material, Path("style.css"), strict=True)
    )
    functional_material = ".toolbar { backdrop-filter: blur(12px); }"
    assert not any(
        issue.startswith("content-blur:")
        for issue in quality_issues(functional_material, Path("style.css"), strict=True)
    )
...
def test_multi_page_static_contract(runner) -> None:
    with tempfile.TemporaryDirectory() as temp:
        ...
        assert "prefers-reduced-motion" in render_module.MOTION_SCRIPT
            ...
            [
                ...
                view_call(narrow),
                {
                    ...
                    "tool_calls": [
                        {
                            "name": "computer_use",
                            "args": {"action": "inspect rendered route", "route": "/"},
                        }
                    ],
                },
                {
                    "status": "DONE",
                    "tool_calls": [
                        {"name": "search_web", "args": {"q": "official responsive UI accessibility guidance"}},
                    ],
                },
                {
                    "status": "DONE",
                    "tool_calls": [
                        {
                            ...
                            "args": {"Url": "https://developer.chrome.com/docs/web-platform/long-animation-frames"},
                        }
                    ],
                },
            ]
...
def test_visual_web_research_order(runner) -> None:
    from hooklib.visual_contract import post_visual_web_research

    calls = [
        {"name": "computer", "args": {"action": "inspect"}},
    ]
    assert post_visual_web_research(calls, 0)[0] is False
    calls.extend(
        [
            {"name": "search_web", "args": {"q": "UI"}},
    ...
    calls.append(
        {
            "name": "read_url_content",
            "args": {"Url": "https://fluent2.microsoft.design/motion"},
        }
    )
    assert post_visual_web_research(calls, 0) == (True, "")
...
def test_failed_view_is_not_read_evidence(runner) -> None:
    with tempfile.TemporaryDirectory() as temp:
        directory = Path(temp)
        workspace = directory / "workspace"
        source = workspace / "src" / "main.py"
        ...
        source.write_text("print('source')\n", encoding="utf-8")
        records = [
            {"type": "USER_INPUT", "content": "/GTG 코드를 검토해줘"},
            {
                "type": "PLANNER_RESPONSE",
                "status": "ERROR",
                "error": "view_file failed",
                "tool_calls": [{"name": "view_file", "args": {"AbsolutePath": str(source)}}],
            },
        ]
        payload = write_transcript(directory, records, "failed-view-conversation")
        payload["workspacePaths"] = [str(workspace)]
        assert not runner.scope_read_complete(payload)
        payload["toolCall"] = {
            "name": "write_to_file",
            "args": {"TargetFile": str(source), "CodeContent": source.read_text(encoding="utf-8")},
        }
        assert runner.decision_for("scope-read-gate", payload)["decision"] == "deny"


def test_strict_read_only_request_blocks_writes(runner) -> None:
    with tempfile.TemporaryDirectory() as temp:
        directory = Path(temp)
        workspace = directory / "workspace"
        workspace.mkdir()
        plan = workspace / "docs" / "plans" / "implementation-plan.md"
        plan.parent.mkdir(parents=True)
        plan.write_text("# Existing plan\n", encoding="utf-8")
        payload = write_transcript(
            directory,
            [{"type": "USER_INPUT", "content": "/GTG 읽기 전용 검토입니다. 절대 수정하지 마세요."}],
            "read-only-write-conversation",
        )
        payload["workspacePaths"] = [str(workspace)]
        payload["toolCall"] = {
            "name": "replace_file_content",
            "args": {"TargetFile": str(plan), "ReplacementContent": "# Changed\n"},
        }
        denied = runner.decision_for("scope-read-gate", payload)
        assert denied["decision"] == "deny"
        assert "읽기·검토·진단 요청" in denied["reason"]
...
def test_stop_snapshot_does_not_expire_scope_gate() -> None:
    with tempfile.TemporaryDirectory() as temp:
        directory = Path(temp)
        workspace = directory / "workspace"
        ...
        for _ in range(6):
            response = invoke_hook_cli("stop-snapshot", payload, environment)
            assert response["decision"] == "continue", response
            assert "재시도 횟수로 우회" in response["reason"]
...
def main() -> None:
    ...
    test_evidence_matrix_requires_local_decisions(runner)
    test_decision_ledger_requires_implementation_and_verification(runner)
    test_contextual_material_gate(runner)
    ...
    test_visual_web_research_order(runner)
    ...
    test_failed_view_is_not_read_evidence(runner)
    test_strict_read_only_request_blocks_writes(runner)
