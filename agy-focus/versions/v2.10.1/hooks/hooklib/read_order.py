...
WORKSPACE_EXPLORER_RE = re.compile(
    ...
    r"\b(?:get|read)[_-]?(?:workspace|file[_-]?tree|directory)\b|\blist_dir\b)",
    re.IGNORECASE,
)
...
def workspace_exploration_denied(payload: dict, status: dict) -> bool:
    ...
    is_workspace_tool = bool(
        re.search(r"\b(?:workspace|tree|directory|folder|list|list_dir|scan|index)\b", name, re.IGNORECASE)
    )
