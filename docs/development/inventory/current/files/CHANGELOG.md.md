...
- 형식: `100644`
- 바이트: 21466
- SHA-256: `7de6bace02c215a66fe1fbf6a36c60eb5a5d8d17b1c537fb3b2edfb381557fde`
- 인코딩: `utf-8`

```
# 3.0.0 — 2026-09-09

2.x의 프롬프트 프로필을 Python 실행 코어로 전면 교체한 판입니다. 알파 세 판을 거쳐 실제 호스트에서 확인한 뒤 내보냅니다.

## 새로 생긴 것

- 완료 조건을 미리 고정하고 등록된 명령을 실제로 실행합니다. 종료 코드·시간·검증 대상 파일 해시를 남기고, 대상 파일이 바뀌면 이전 성공을 무효로 만듭니다.
- `certify`가 완료 주장을 실행 명령·종료 코드·파일 지문에 묶고, `replay`가 상태 DB 없이 그 문서만으로 재현합니다. 판정은 `pass`·`invalid`·`inconclusive`이며 종료 코드는 0·1·2입니다.
- 검사가 실제로 실행한 코드 파일을 관찰해, 통과했지만 한 번도 실행하지 않은 검증 대상을 그대로 보고합니다. 관찰할 수 없는 언어가 범위에 있으면 미실행을 주장하지 않고 무엇이 판단을 막았는지 밝힙니다.
- 세 호스트를 지원합니다. Antigravity 데스크톱·IDE, Antigravity CLI(`agy`), Gemini CLI. 호스트별 차이는 `gtg/platforms.py` 한 곳에만 있습니다.
- 세션이 끊겨도 작업이 남습니다. 새 세션에서 `tasks`로 찾아 `attach`하면 목표·검사 결과·메모를 이어받습니다.
- 설치기가 호스트를 감지해 한 명령으로 설치·검사·제거합니다. 파일 이동을 미리 기록해 도중에 죽어도 다음 실행이 정리합니다.

## 이 판에서 고친 것

- 재부팅으로 PID가 재사용되면 `recover`가 영구히 거부되어 작업이 교착됐습니다. 실행 중 여부를 파일 잠금으로 판단하도록 바꿨습니다. 검사 프로세스가 잠금을 물려받아 소유자만 죽은 경우도 같은 신호로 드러납니다.
- 검사가 실행 중일 때 종료 훅이 완료를 조용히 허용했습니다. 재개 예산은 그대로 쓰지 않으면서 남은 검사와 복구 방법을 알리도록 분리했습니다.
- 미실행 범위 보고가 이유 없이 비었습니다. 셸 스크립트 하나만 범위에 있어도 목록 전체가 `null`이 됐습니다. 무엇이 막았는지 `unobservable_watch`로 밝힙니다.
- Antigravity CLI에서 파일을 경로에 놓기만 하고 `agy plugin install`을 하지 않아, 설치해도 훅이 발행되지 않았습니다. 설치기가 등록까지 합니다.
- 평가 도구가 작업공간에 설치된 GTG 자기 모듈을 "코드를 실행했다"로 세어 GTG에 유리하게 기울어 있었습니다.

## 호환성

- 증명서 스키마가 판 3입니다. 이전 판 증명서는 거부되므로 `certify`로 다시 만드세요.
- 개발 문서 폴더가 `docs/rebuild`에서 `docs/development`로 바뀌었습니다.

## 알아 둘 것

같은 모델에서 GTG를 켜고 끈 64회 비교에서 짧은 작업의 산출물 결과는 같았습니다. GTG가 값을 하는 지점은 한 턴에 끝나지 않는 작업입니다. 측정치와 확인하지 못한 항목은 [확인한 것과 확인하지 못한 것](docs/verification/README.md)에 있습니다.
