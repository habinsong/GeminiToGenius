# `agy-focus/versions/v2.15.0/scripts/ui_render_audit.py`

- 형식: `100644`
- 바이트: 7767
- SHA-256: `affba739fc3281fb31ece0fe4f568bb5cbdf33dc8cc5b9d2be27a25307cb7018`
- 인코딩: `utf-8`
...
MOTION_SCRIPT = """(() => {
  ...
  const computed = Array.from(document.querySelectorAll("body *")).map(element => getComputedStyle(element));
  const active = document.getAnimations().filter(animation => animation.playState !== "idle");
  const infinite = computed.filter(style => style.animationIterationCount === "infinite").length;
  const cssMotion = /(?:@keyframes|animation(?:-name|-duration|-timing-function|-iteration-count)?\\s*:|transition(?:-duration|-property|-timing-function)?\\s*:)/i.test(styleText);
  const activeAnimations = computed.filter(style => style.animationName && style.animationName !== "none").length;
  return {
    hasMotion: cssMotion || active.length > 0 || activeAnimations > 0,
    cssMotion,
    activeAnimations: active.length,
    computedAnimations: activeAnimations,
    infiniteAnimations: infinite,
    reducedMotionRule: /prefers-reduced-motion\\s*:\\s*reduce/i.test(styleText),
