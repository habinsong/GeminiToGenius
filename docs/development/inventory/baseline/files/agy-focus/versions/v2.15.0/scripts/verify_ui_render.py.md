# `agy-focus/versions/v2.15.0/scripts/verify_ui_render.py`

- 형식: `100644`
- 바이트: 10136
- SHA-256: `f1481ba7f390577e3aa500e2e55e47c28c7fef4e5bf48811694c14fb1b75ff47`
- 인코딩: `utf-8`
...
from motion_audit import reduced_motion_audit
from ui_render_audit import MOTION_SCRIPT, OVERFLOW_SCRIPT, PERFORMANCE_SCRIPT, UI_AUDIT_SCRIPT, keyboard_focus_audit
...
def render_viewport(client: DevTools, url: str, output_dir: Path, width: int, height: int, label: str) -> dict:
    ...
    motion = evaluate(client, MOTION_SCRIPT)
    ...
    reduced_motion = reduced_motion_audit(client, evaluate, wait_for_page)
    if motion.get("hasMotion") and not motion.get("reducedMotionRule"):
        audit.setdefault("failures", []).append({"type": "reduced-motion-contract"})
    if not reduced_motion.get("prefersReducedMotion"):
        audit.setdefault("failures", []).append({"type": "reduced-motion-emulation"})
    metrics.update(
        {
            "width": width,
            "height": height,
            ...
            "motion": {"default": motion, "reduced": reduced_motion},
        }
    )
    return metrics
