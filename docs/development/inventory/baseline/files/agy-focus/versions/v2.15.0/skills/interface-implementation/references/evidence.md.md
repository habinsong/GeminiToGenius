# `agy-focus/versions/v2.15.0/skills/interface-implementation/references/evidence.md`

- 형식: `100644`
- 바이트: 7330
- SHA-256: `036e0b6786b83b433de6702d8e163ec226370997ad4550c82220200caba1da36`
- 인코딩: `utf-8`
...
| 근거 | 구현 결정 | 구현·검증 게이트 |
|---|---|---|
| [Apple WWDC26 Design](https://developer.apple.com/wwdc26/guides/design/) | readability·accessibility·창 크기 적응을 실제 콘텐츠 위계에 연결 | 320·768·1280, 긴 문자열, 지역화, 접근성 상태에서 읽기 순서를 확인 |
| [Android Adaptive do's and don'ts](https://developer.android.com/develop/adaptive-apps/guides/adaptive-dos-and-donts), [Window size classes](https://developer.android.com/develop/adaptive-apps/guides/use-window-size-classes?hl=en) | 동적 창 크기로 reflow·reveal·multi-pane을 고르고 넓은 화면을 무조건 stretch하지 않음 | compact·medium·expanded와 resize·split-window·외부 입력을 테스트 |
| [Microsoft Fluent 2 Motion](https://fluent2.microsoft.design/motion), [Fluent 2 Accessibility](https://fluent2.microsoft.design/accessibility) | 목적 있는 짧은 motion, no-motion, focus·ARIA 대체를 함께 구현 | reduced motion에서도 사용 가능성·focus·layout-triggering 속성을 확인 |
| [Atlassian Motion](https://atlassian.design/foundations/motion) | 고빈도 interaction과 큰 transition을 의미 토큰·빈도·요소 크기로 구분 | 50–150ms·150–400ms 기준을 무비판적 숫자가 아닌 flow·프레임 근거와 대조 |
| [Adobe Spectrum 2](https://s2.spectrum.adobe.com/) | 플랫폼·창·선호에 맞춰 contextual·cohesive·performant하게 적응 | scale·contrast·color preference와 desktop·web·mobile에서 정보 손실을 검사 |
| [IBM Carbon Accessibility](https://carbondesignsystem.com/guidelines/accessibility/overview/) | WCAG AA, 평이한 카피, 색상 외 상태, 자동 재생·깜박임 회피 | 대비·focus·notification 맥락·복구·시간 제한을 확인 |
| [Shopify Polaris Accessibility](https://polaris-react.shopify.com/foundations/accessibility) | 의미 HTML·기존 접근성 컴포넌트·focus 관리 우선 | 키보드 순서·접근 가능한 이름·focus 복귀·landmark를 조작 검증 |

이 자료들은 브랜드 스타일 복제 목록이 아닙니다. 구현 순서는 렌더 → Computer Use·vision 실제 화면 관찰 → 관찰 뒤 `search_web`와 공식 `read_url_content` 비교 → 수정 및 재검증으로 고정합니다.
```
