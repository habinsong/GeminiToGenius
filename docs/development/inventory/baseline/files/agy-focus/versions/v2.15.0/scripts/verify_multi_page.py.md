# `agy-focus/versions/v2.15.0/scripts/verify_multi_page.py`

- 형식: `100644`
- 바이트: 6710
- SHA-256: `946c312021923744580c4443fd8b8190f8df9d3f061c6e0e7de0d1901371cb5f`
- 인코딩: `utf-8`
...
def _has_reduced_motion_override(text: str) -> bool:
    return bool(
        re.search(
            r"@media\s*\(\s*prefers-reduced-motion\s*:\s*reduce\s*\)\s*\{"
            r"[\s\S]{0,3000}?"
            r"(?:animation(?:-[\w-]+)?\s*:\s*(?:none|0s?)|"
            r"transition(?:-[\w-]+)?\s*:\s*(?:none|0s?))",
            text,
            re.IGNORECASE,
        )
    )
...
def static_checks(root: Path, html_paths: list[Path]) -> tuple[list[str], dict[str, bool]]:
    ...
    for path in html_paths:
        ...
        if not re.search(r"<html\b[^>]*\blang\s*=", text, re.IGNORECASE):
            failures.append(f"{path.relative_to(root)}:missing:html-lang")
        if not re.search(r"<meta\b[^>]*charset\s*=", text, re.IGNORECASE):
            failures.append(f"{path.relative_to(root)}:missing:charset")
        if not _tag_text(text, "title"):
            failures.append(f"{path.relative_to(root)}:empty:title")
        if not _tag_text(text, "h1"):
            failures.append(f"{path.relative_to(root)}:empty:h1")
    titles = [_tag_text(path.read_text(encoding="utf-8"), "title").casefold() for path in html_paths]
    headings = [_tag_text(path.read_text(encoding="utf-8"), "h1").casefold() for path in html_paths]
    for value, label in ((titles, "title"), (headings, "h1")):
        duplicates = sorted({item for item in value if item and value.count(item) > 1})
        if duplicates:
            failures.append(f"routes:duplicate:{label}:{','.join(duplicates[:3])}")
    contracts = {
        "reducedMotion": _has_reduced_motion_override(aggregate),
        "visibleFocus": bool(
            re.search(
                r":focus-visible[^{}]*\{[^{}]*(?:outline|box-shadow)\s*:",
                aggregate,
