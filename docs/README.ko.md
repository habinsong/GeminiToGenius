...
`/GTG`는 요청이 끝날 때까지 코드 우선 탐색, 파일 전체 읽기, 원격 저장소 1차 근거, UI 소스 쓰기 전 계획, 웹페이지·랜딩·정적 페이지의 2개 이상 경로, 320px·768px·데스크톱 브라우저 렌더, 쓰기 후 검증을 유지합니다.

## `GEMINI.md`를 항상 두는 이유

- Antigravity는 `~/.gemini/GEMINI.md`를 [전역 규칙](https://antigravity.google/docs/ide/rules)으로 읽습니다.
- v2.5.0 진입점은 5,605자입니다. 라우팅·안전·근거·계획·완료·UI 감사 게이트만 둡니다.
...
- UI 방향은 서로 다른 공식 출처를 최소 2개 읽은 뒤 evidence matrix에 선택·구현 영향·검증 기준을 남깁니다.
