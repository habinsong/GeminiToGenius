...
- 형식: `100644`
- 바이트: 10076
- SHA-256: `288f4e599b38a02b3518c7307591be98503e44b4138f10094991cfbe52c5ec9a`
- 인코딩: `utf-8`
...
발행된 최신 릴리스는 **3.0.0-alpha.1 사전 릴리스**이며 저장소의 현재 버전은 **3.0.0-alpha.3(개발 중, 미발행)**입니다. 기존 프롬프트 프로필을 Python 실행 코어·호스트별 훅·작업별 스킬로 재개발하고 있습니다. 로컬 실행과 Antigravity의 발견·초기 훅·자동 스킬 선택을 확인했으며, Gemini 3.8 Flash High로 선택된 단일 오류 수정 사례가 독립 검사를 통과했습니다. 다른 모델과 다양한 작업, 경쟁 도구 대비 품질은 아직 검증하지 않았습니다.
...
복잡한 작업에서는 에이전트가 목표·완료 조건·검증 명령을 등록합니다. 실행기는 명령을 직접 실행해 종료 코드와 파일 해시를 기록합니다. 검사 후 코드가 바뀌면 이전 성공을 무효화합니다. 상태는 프로젝트의 `.gtg/`에 저장되며 이 폴더는 자기 자신을 무시하는 `.gitignore`를 함께 만듭니다. 사용자의 `.gitignore`는 바꾸지 않고, 같은 이름의 기존 파일도 덮어쓰지 않습니다.
...
새 세션에서 이어가려면 에이전트가 저장된 작업 후보와 원래 명세를 확인한 뒤 같은 작업에 연결합니다. 목표·검증 결과·메모와 현재 파일을 유지하며, 과거의 성공을 현재 검증으로 혼동하지 않습니다. 작업 조회·연결은 로컬 검사와 실제 새 세션의 제한된 재개 사례에서 확인했습니다. 기존 호스트의 종료나 동시 파일 편집 제어는 호스트에서 확인해야 합니다.

훅은 연결된 작업의 미검증 상태를 알리고 자연 종료를 기본 두 번까지 재개하도록 구현했습니다. 여러 단계가 필요한 긴 작업은 에이전트가 작업 명세의 `max_resumes`를 8 이하로 올릴 수 있습니다. 자동 재개는 사용자 쿼터를 사용하므로 기본값은 낮게 두었습니다. 사용자 중단·오류·백그라운드 작업을 무시하고 계속 호출하지 않습니다. 훅에서 LLM을 추가 호출하지 않습니다.
...
검사를 실행할 때 어떤 코드 파일이 실제로 실행됐는지도 함께 기록합니다. Python과 Node를 관찰하며 하위 프로세스도 포함합니다. 증명서의 `unverified_scope`에는 **통과했지만 어떤 검사도 실행하지 않은 검증 대상**이 나옵니다. 통과 자체가 근거의 전부가 아니라는 것을 사실로 보여 줍니다. 파일 단위 관찰이며 줄·분기 커버리지가 아닙니다. 실행을 관찰하지 못했거나 TypeScript·Go 같은 관찰 불가 언어가 범위에 있으면 미실행을 주장하지 않습니다.

종료 코드 0과 기록된 지문이 모두 재현될 때만 `reconstructed: true`입니다. 판정은 세 값이며 재현과 확인 범위가 모두 충족돼야 `pass`(종료 코드 0)입니다. 재현이 깨지면 `invalid`(1), 확인하지 못한 범위가 남으면 `inconclusive`(2)로 구분합니다. 본문을 고치면 `digest`가 달라져 거부하고, 검증 대상 파일이 바뀌었으면 `reason: "fingerprint"`로 표시합니다. 미검증 작업의 `certify`는 종료 코드 1과 `verified: false`를 반환합니다. 증명서는 등록된 검사 범위의 재현 가능성만 보이며 검사 설계의 적절성은 별도 판단입니다.
