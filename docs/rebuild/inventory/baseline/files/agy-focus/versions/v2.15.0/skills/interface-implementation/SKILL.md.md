# `agy-focus/versions/v2.15.0/skills/interface-implementation/SKILL.md`

- 형식: `100644`
- 바이트: 5148
- SHA-256: `716966dbbf24ff6a55d9183c2af92b6d753bfdeb14af16174204e533f0849fca`
- 인코딩: `utf-8`
...
4. 가짜 윈도우 창틀(트래픽 라이트 점, 윈도우 타이틀바), 조잡한 앱 흉내, 좌측 어두운 사이드바 + 우측 1px 사각 테두리 박스(Card/Panel) 무한 나열 덤프, 형광 초록·네온사인 발광 색상, 그라데이션, 글로우, 의미 없는 pill, 반복 카드를 전면 금지합니다.
...
12. 반복 UI와 상태·도메인·IO를 한 컴포넌트에 몰지 않고 `architecture-boundaries`를 따릅니다. 컴포넌트 분리는 책임과 재사용 근거가 있을 때만 합니다. `design-plan.md`의 Decision ID마다 구현 파일·책임·검증 방법이 실제로 존재하는지 확인합니다.
13. "AI 특유의 서술 텍스트"와 "AI 기술 서술 텍스트"를 전면 배제합니다. 특별한 사유(버튼 이름 등)가 없는 한 단답형 나열을 금지하고 온전한 문장형 종결어미(~합니다, ~해요 등)를 사용하며, 비전공자도 단번에 알아들을 수 있는 쉬운 일상 언어로 작성합니다.
14. 웹 UI는 기본 제공 `scripts/verify_ui_render.py`로 320px·768px·1280px 렌더와 수평 overflow를 계측하고 모든 스크린샷을 직접 엽니다. 이 검사는 문서 언어·charset·title/h1/main·이미지 alt·접근 가능한 이름·24 CSS px 조작 영역·실제 focus 스타일·CLS·Long Animation Frames·실제 모션·reduced-motion emulation을 반환합니다.
15. 페이지 목적과 사용자 흐름이 둘 이상이면 다중 페이지를 계획하고 구현합니다. 각 경로는 고유한 `title`·`h1`·`main`과 실제 링크를 가져야 하며 `verify_multi_page.py`로 모든 경로를 320px·768px·1280px에서 렌더합니다.
