# `agy-focus/versions/v2.15.0/skills/interface-planning/references/evidence.md`

- 형식: `100644`
- 바이트: 12918
- SHA-256: `ff62bc244b41eb4bc64e1e9ade890697ac734e15c50d0a9f783cb1461079eae3`
- 인코딩: `utf-8`
...
| 근거 | 계획 결정 | 구현·검증 게이트 |
|---|---|---|
| [Android adaptive do's and don'ts](https://developer.android.com/develop/adaptive-apps/guides/adaptive-dos-and-donts), [Material 3 Adaptive](https://developer.android.com/jetpack/androidx/releases/compose-material3-adaptive) | 디바이스 이름이 아니라 실제 창 크기와 compact·medium·expanded 관계로 reflow·reveal·pane을 선택하고 무조건 stretch하지 않음 | 320·768·1280에서 읽기 순서·pane·긴 문자열·입력 장치·overflow를 확인 |
| [Apple Materials](https://developer.apple.com/design/human-interface-guidelines/materials), [Adopting Liquid Glass](https://developer.apple.com/documentation/TechnologyOverviews/adopting-liquid-glass) | 재료는 기능·탐색 계층에 필요한 만큼만 쓰고 콘텐츠 면의 장식으로 반복하지 않음 | underlying content 가림, reduced transparency·high contrast fallback, focus·상태 식별을 실제 화면에서 확인 |
| [Design Theater](https://arxiv.org/abs/2607.22928), [Bridging Gulfs](https://arxiv.org/abs/2601.19171) | 자연어 설명을 구현으로 간주하지 않고 Decision ID·사용자 결과·근거·구현 위치·검증 방법을 장부로 고정 | 계획의 각 행을 실제 파일·selector·검증 출력과 대조하고 선언만 있는 기능을 실패 처리 |
| [FlowEval](https://arxiv.org/abs/2605.04165), [GUIDE](https://research.google/pubs/guide-a-benchmark-for-user-context-understanding-and-assistance-in-gui-workflow-videos/) | 화면 캡처 한 장이 아니라 실제 사용자의 route·행동·상태 전이·복구 흐름을 검증 기준으로 선택 | primary flow와 recovery path를 브라우저에서 재생하고 route/state trace를 독립 기준과 비교 |
...
## v2.10 추가 대조 — 2026-08-03

| 근거 | 계획 결정 | 구현·검증 게이트 |
|---|---|---|
| [Apple WWDC26 Design](https://developer.apple.com/wwdc26/guides/design/) | 플랫폼 공통 언어를 복제하지 않고 readability·accessibility·창 크기 적응을 실제 콘텐츠 위계에 연결 | 320·768·1280과 긴 문자열·지역화·접근성 상태에서 동일 기능의 읽기 순서를 확인 |
| [Android Adaptive do's and don'ts](https://developer.android.com/develop/adaptive-apps/guides/adaptive-dos-and-donts), [Window size classes](https://developer.android.com/develop/adaptive-apps/guides/use-window-size-classes?hl=en) | 디바이스 이름 대신 동적 창 크기를 기준으로 reflow·reveal·multi-pane을 선택하고 넓은 화면을 무조건 stretch하지 않음 | compact·medium·expanded 경계와 resize·split-window·외부 입력을 실제로 테스트 |
| [Microsoft Fluent 2 Motion](https://fluent2.microsoft.design/motion), [Fluent 2 Accessibility](https://fluent2.microsoft.design/accessibility) | motion은 목적·초점·자연스러운 짧은 시간으로 제한하고 no-motion·ARIA 대체를 함께 설계 | focus가 화면 밖으로 이동하지 않는지, reduced motion에서 사용 가능성이 유지되는지, layout-triggering 속성을 애니메이션하지 않는지 확인 |
| [Atlassian Motion](https://atlassian.design/foundations/motion) | 고빈도 interaction은 50–150ms, 큰 transition은 150–400ms의 의미 토큰으로 선택하고 단일 focal point를 유지 | 빈도·요소 크기·진입/이탈·감속 설정을 flow trace와 실제 프레임 진단으로 대조 |
| [Adobe Spectrum 2](https://s2.spectrum.adobe.com/) | 화면 폭·플랫폼·사용자 선호에 따라 contextual·cohesive·performant하게 flex하되 사람의 작업을 먼저 둠 | desktop·web·mobile 상태와 scale·contrast·color preference에서 정보 손실·대비·조작 가능성을 확인 |
| [IBM Carbon Accessibility](https://carbondesignsystem.com/guidelines/accessibility/overview/) | WCAG AA, 평이한 언어, 색상 외 상태, 복잡한 문장·자동 재생·깜박임 회피를 제품 계약에 포함 | 본문·컨트롤 대비, 키보드 focus, notification의 맥락·복구·시간 제한을 검사 |
| [Shopify Polaris Accessibility](https://polaris-react.shopify.com/foundations/accessibility) | 의미 HTML·접근 가능한 기존 컴포넌트·focus 관리부터 선택하고 장식을 새로 발명하지 않음 | 키보드 순서·이름·focus 복귀·화면 reader 경계를 실제 조작으로 검증 |

이 자료들은 특정 브랜드 모양을 복제하는 지시가 아닙니다. 공통으로 반복되는 사용자 관점의 검증 항목만 추출합니다. 반응형은 창 크기와 콘텐츠 관계를, motion은 목적·빈도·초점·감속을, 시각 방향은 실제 제품 정보와 상태를 기준으로 선택합니다. 렌더 뒤 Computer Use·vision 관찰을 하고, 관찰 뒤 `search_web`와 공식 `read_url_content`로 한 번 더 대조한 기록이 없으면 UI 완료로 쓰지 않습니다.
```
